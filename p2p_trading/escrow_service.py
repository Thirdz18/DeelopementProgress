...
class P2PEscrowService:
    ...
    def backfill_stuck_ads(self) -> int:
        """Backfill ads and trades that are stuck at 'submitted' or 'pending_user_signature'.
        
        Queries the blockchain to verify if ads/trades exist and are active,
        then updates the DB accordingly.
        ...
        try:
            # BACKFILL STUCK ADS
            # Get all stuck ads (status = 'submitted' OR 'pending_user_signature')
            stuck_ads = (
                self.supabase.table("p2p_orders")
            ...
            # Get all stuck trades (status = 'submitted' OR 'pending_user_signature')
            stuck_trades = (
                self.supabase.table("p2p_trades")
            ...
            if stuck_trades.data:
                ...
                for trade in stuck_trades.data:
                    ...
                    try:
                        # Query the blockchain for the trade
                        trade_view = self.contract.get_trade(trade_id_hex)
                        
                        if trade_view and trade_view.exists:
                            # Trade exists on-chain, update based on status
                            # Map contract status to DB status
                            contract_status = trade_view.status
                            
                            # Status codes from contract:
                            # 0=none, 1=payment_pending, 2=awaiting_release, 
                            # 3=completed, 4=cancelled, 5=expired, 6=disputed, 7=refunded
                            if contract_status == 1:
                                new_status = "payment_pending"
                            elif contract_status == 2:
                                new_status = "awaiting_release"
                            elif contract_status == 3:
                                new_status = "completed"
                            elif contract_status == 4:
                                new_status = "cancelled"
                            elif contract_status == 5:
                                new_status = "expired"
                            elif contract_status == 6:
                                new_status = "disputed"
                            elif contract_status == 7:
                                new_status = "refunded"
                            else:
                                new_status = "payment_pending"  # Default to payment_pending for status 0
                            ...
                            self._log_action(
                                ...
                                notes=f"Backfilled to {new_status} from blockchain",
                            )
                            
                            updated_count += 1
                            logger.info(f"Backfilled trade {trade['trade_id']} ({trade_id_hex}) -> {new_status}")
                        else:
                            # Trade not found on-chain - mark as cancelled
                            db.table("p2p_trades").update({
                            ...
                            self._log_action(
                                ...
                                trade_id=trade["trade_id"],
                                actor=trade.get("buyer_wallet"),
                                notes="Marked as cancelled (not found on-chain)",
                            )
                            
                            updated_count += 1
                            logger.info(f"Cancelled stale trade {trade['trade_id']} ({trade_id_hex})")
